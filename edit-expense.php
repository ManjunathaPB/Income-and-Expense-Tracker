...
if (strlen($_SESSION['user'] == 0)) {
    ...
    if (isset($_GET['id'])) {
        ...
        $query = mysqli_query($conn, "SELECT * FROM tblexpense WHERE ID='$expenseId'");
    ...
    if (isset($_POST['update'])) {
        ...
        $updateQuery = mysqli_query($conn, "UPDATE tblexpense SET ExpenseItem='$newExpenseItem', ExpenseCost='$newExpenseCost', ExpenseDate='$newExpenseDate' WHERE ID='$expenseId'");
        if ($updateQuery) {
            echo "<script>alert('Record successfully updated');</script>";
            echo "<script>window.location.href='manage-expense1.php'</script>";
        } else {
            ...
            <form method="POST" class="expense-form">
                ...
                <div class="sectionmenu">
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control" id="expense_date" name="expense_date" value="<?php echo $expenseData['ExpenseDate']; ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="expense_item">Expense Item</label>
                        <input type="text" class="form-control" id="expense_item" name="expense_item" value="<?php echo $expenseData['ExpenseItem']; ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="expense_cost">Expense Cost</label>
                        <input type="number" class="form-control" id="expense_cost" name="expense_cost" value="<?php echo $expenseData['ExpenseCost']; ?>" step="10" required>
                    </div>
