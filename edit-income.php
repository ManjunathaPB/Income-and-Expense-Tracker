...
if (strlen($_SESSION['user'] == 0)) {
    ...
    if (isset($_GET['id'])) {
        ...
        $query = mysqli_query($conn, "SELECT * FROM tblincome WHERE ID='$expenseId'");
        $expenseData = mysqli_fetch_assoc($query);
    }

    if (isset($_POST['update'])) {
        // Retrieve updated data from form
        $newExpenseItem = $_POST['expense_item'];
        $newExpenseCost = $_POST['expense_cost'];
        $newExpenseDate = $_POST['expense_date'];

        // Perform update query
        $updateQuery = mysqli_query($conn, "UPDATE tblincome SET IncomeName='$newExpenseItem', IncomeAmt='$newExpenseCost', IncomeDate='$newExpenseDate' WHERE ID='$expenseId'");
        if ($updateQuery) {
    ...
    <body>
                ...
                <div class="menu">
                    ...
                    <div class="item"><a class="sub-btn"><i class="fas fa-bars"></i>Expenses<i class="fas fa-angle-right dropdown"></i></a>
                        <div class="sub-menu">
                            ...
                            <div class="sub-items"> <a href="manage-expense1.php" style="background-color: white; border:1px solid black;">View expenses</a></div>
                        </div>
                    </div>
        ...
        <div class="section">
            <form method="POST" class="expense-form">
                ...
                <!-- <h2>Edit Expense</h2> -->
                <div class="header">Edit Income</div>
                <div class="sectionmenu">
                    <div class="form-group">
                        <label for="expense_date">Income Date</label>
                        <input type="date" class="form-control" id="expense_date" name="expense_date" value="<?php echo $expenseData['IncomeDate']; ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="expense_item">Income Name</label>
                        <input type="text" class="form-control" id="expense_item" name="expense_item" value="<?php echo $expenseData['IncomeName']; ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="expense_cost">Income Amount</label>
                        <input type="number" class="form-control" id="expense_cost" name="expense_cost" value="<?php echo $expenseData['IncomeAmt']; ?>" step="10" required>
                    </div>
